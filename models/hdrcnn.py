...
class SoftConvNotLearnedMaskUNet(BaseNetwork):
    ...
    def forward(self, input, input_mask):
        ...
        for i in range(self.layer_size, 0, -1):
            ...
            # --- Fix: align encoder and decoder feature sizes ---
            enc_feat = h_dict[enc_h_key]
            enc_mask = h_mask_dict[enc_h_key]

            min_h = min(h.shape[2], enc_feat.shape[2])
            min_w = min(h.shape[3], enc_feat.shape[3])

            h = h[:, :, :min_h, :min_w]
            h_mask = h_mask[:, :, :min_h, :min_w]
            enc_feat = enc_feat[:, :, :min_h, :min_w]
            enc_mask = enc_mask[:, :, :min_h, :min_w]
            # ---------------------------------------------------
